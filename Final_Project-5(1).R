...
df%>%
  ggplot(aes(x=temperature,y=load))+
  geom_point(size=0.5)+
  geom_smooth()+
  theme_bw()+
  ggtitle('The correlation between temperature and load')
...
res_arima = Arima(resid_ts, order = c(2,0,1), seasonal = c(1,0,0)) # need some time
...
ggplot(data = result, aes(x = Date, y = MAPE))+
  ...
  ggtitle("MAPE in 2017 Jan，Feb & March")+
  theme_bw()
